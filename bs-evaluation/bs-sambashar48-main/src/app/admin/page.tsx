...
export default function AdminPage() {
  ...
  // Cloud sync toggle
  const [togglingCloudUserId, setTogglingCloudUserId] = useState<string | null>(null);
  ...
  // ======== Toggle Cloud Sync Permission ========
  const handleToggleCloudSync = async (targetUser: User) => {
    const newState = !(targetUser.cloud_sync_enabled ?? false);
    setTogglingCloudUserId(targetUser.id);
    setError('');
    try {
      const res = await fetch(`/api/admin/users/${targetUser.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cloudSyncEnabled: newState }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || 'فشل في تحديث صلاحية المزامنة');
        return;
      }
      await fetchUsers();
    } catch {
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50/80 hover:bg-gray-50/80">
                    ...
                    <TableHead className="text-center font-semibold text-gray-600 px-4 py-3 hidden sm:table-cell">
                      مزامنة سحابية
                    </TableHead>
                    ...
                    <TableRow key={u.id} className="group">
                      ...
                      <TableCell className="px-4 py-3 text-center hidden sm:table-cell">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleToggleCloudSync(u)}
                          disabled={u.role === 'admin' || togglingCloudUserId === u.id}
                          title={u.role === 'admin' ? 'المشرف يملك كل الصلاحيات تلقائياً' : (u.cloud_sync_enabled ? 'تعطيل المزامنة السحابية' : 'تفعيل المزامنة السحابية')}
                        ...
                        >
                          ...
                          <span className="hidden lg:inline">{u.cloud_sync_enabled ? 'مفعّل' : 'معطّل'}</span>
                        </Button>
                      </TableCell>
