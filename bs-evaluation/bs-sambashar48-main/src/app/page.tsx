...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = bsStorage.onConnectionChange((status) => {
      ...
      if (status === 'online' && !onlineNotifiedRef.current && canSyncCloud()) {
        ...
        toast.success('الإنترنت متاح للحفظ السحابي', { duration: 3000 });
      }
      if (status === 'offline') {
  ...
  const getCurrentLocalProject = async (): Promise<LocalProject | null> => {
    ...
    return {
      id: currentProjectId,
      userId: user?.id || '',
      name: currentProject?.name || 'مشروع',
      isCurrent: true,
      data: projectData as unknown as Record<ProjectSection, Record<string, unknown>>,
      images: {},
  ...
  const handleRecallProject = async (projectId: string) => {
    try {
      ...
      if (!project) {
        toast.error('فشل استدعاء المشروع — غير موجود محلياً');
        return;
      }
      ...
      toast.success(`تم استدعاء المشروع ${project.name}`);
      setShowProjectsPanel(false);
      setSidebarOpen(false);
    } catch {
      toast.error('فشل استدعاء المشروع');
    }
  };
  ...
  const handleSaveLocal = async () => {
    ...
    try {
      ...
      if (!project) {
        toast.error('لا يوجد مشروع للحفظ');
        return;
      }
  ...
  const handleSaveAsFile = async () => {
    ...
    try {
      ...
      setLastSaveTime(now);
      setLastSaveTimeDisplay(CacheManager.timeAgo(now));
      toast.success('تم حفظ المشروع كملف .bsproj');
    } catch {
      toast.error('فشل حفظ المشروع كملف');
    }
  };
  ...
  const handleSaveCloud = async () => {
    ...
    if (!canSyncCloud()) {
      toast.error('ليس لديك صلاحية الحفظ السحابي');
      return;
    }
    try {
      const project = await getCurrentLocalProject();
      ...
      setLastSaveTime(now);
      setLastSaveTimeDisplay(CacheManager.timeAgo(now));
      toast.success('تم الحفظ السحابي بنجاح');
    } catch {
  ...
  const handleImportFile = async () => {
    try {
      ...
      if (project) {
        ...
        toast.success(`تم استيراد المشروع "${project.name}" بنجاح`);
      }
    } catch (err) {
  ...
  const handleSyncNow = async () => {
    ...
    if (!canSyncCloud()) {
      toast.error('ليس لديك صلاحية المزامنة');
      return;
    }
    try {
      setIsSyncing(true);
      ...
      if (failCount === 0) {
        toast.success(`تمت مزامنة ${successCount} مشروع بنجاح`);
      } else {
        toast.error(`تمت مزامنة ${successCount} مشروع، فشل ${failCount} مشروع`);
      }
  ...
  const renderLastSaveTime = () => {
    ...
    return (
      <div className="text-[10px] text-muted-foreground mb-2 text-center" dir="rtl">
        آخر حفظ: {lastSaveTimeDisplay}
      </div>
    );
  };
              ...
              {projects.map((p: { id: string; name: string; is_current: boolean; lastSavedAt?: string | null; isDirty?: boolean }) => (
                ...
                >
                  ...
                  >
                    ...
                    {p.isDirty && (
                      <div className="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0" title="غير مزامن" />
                    )}
                  </button>
                  ...
                  {projects.map((p: { id: string; name: string; is_current: boolean; lastSavedAt?: string | null; isDirty?: boolean }) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {p.isDirty && (
                          <div className="w-2 h-2 rounded-full bg-amber-500 shrink-0" title="غير مزامن" />
                        )}
                      </button>
