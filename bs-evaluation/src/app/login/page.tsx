...
const BUILD_VERSION = '20260516-v6';

// Expected auth store version — must match AUTH_STORE_VERSION in stores/index.ts
// If the stored version is older, we clear stale auth data to prevent hydration errors
const EXPECTED_AUTH_STORE_VERSION = 2;
...
export default function LoginPage() {
  ...
  // Only clears stale data when the store version changes,
  // preserving PWA functionality (Service Worker + caches)
  // ─────────────────────────────────────────────────────
  useEffect(() => {
    const cleanup = async () => {
      try {
        // 1. Clear stale Zustand auth store from localStorage
        // (prevents hydration of corrupted auth state from old sessions)
        // Only clears if version mismatch — preserves valid sessions
        const staleAuth = localStorage.getItem('bs-auth');
        if (staleAuth) {
          try {
            const parsed = JSON.parse(staleAuth);
            // If the stored version is older than expected, wipe it
            if (parsed?.version !== undefined && parsed.version < EXPECTED_AUTH_STORE_VERSION) {
        ...
        // 2. NOTE: We do NOT unregister Service Workers or clear Cache API here.
        // This preserves PWA functionality (offline support, caching).
        // The Service Worker (Serwist) handles its own cache updates via versioning.
        // SW cleanup should only happen during a major version upgrade,
        // which is handled by ServiceWorkerRegistrar.tsx.
      } catch {
